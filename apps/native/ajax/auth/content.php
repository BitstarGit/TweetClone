...
if ($action == "login") {
    ...
    foreach ($user_data_fileds as $field_name => $field_val) {
        if ($field_name == 'fname') {
            ...
            if (empty($field_val)) {
                ...
                break;
            } else if (filter_var($field_val, FILTER_VALIDATE_EMAIL) != true || len($field_val) > 55) {
                $data['err_code'] = "invalid_email";
                break;
    ...
    if (empty($data['err_code'])) {
        ...
        if ($cl['config']['acc_validation'] == 'off') {
            ...
            if(cl_follow($user_id, 1)) {
                $data['status'] = 200;
                $data['total_following'] = 1;
                $udata = cl_raw_user_data(1);

                cl_update_user_data($user_id, array(
                    'following' => $data['total_following']
                ));

                cl_update_user_data(1, array(

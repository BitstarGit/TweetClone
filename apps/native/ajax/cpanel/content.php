...
else if($action == 'recover_reported_post') {
    $data['status']   = 404;
    $data['err_code'] = 0;
    $reportId          = fetch_or_get($_POST['reportId'], 0);

    $data['status_code'] = (cl_delete_report_data($reportId) == true) ? 1 : 0;
    $data['status']      = 200;
}

    ...
    if (cl_queryset($tags)) {
    	foreach ($tags as $tag_data) {
    		...
    		if (not_empty($keyword)) {
    			$tag_data['hashtag'] = str_replace($keyword, cl_html_el('span',$keyword,array('class' => 'query-match')), $tag_data['hashtag']);
    		}
...
function cl_search_people($keyword = "", $offset = false, $limit = 10) {
    ...
    if (cl_queryset($query_result)) {
        foreach ($query_result as $row) {
            ...
            if (not_empty($keyword)) {
            	$row['about']    = str_replace($keyword, cl_html_el('span',$keyword,array('class' => 'query-match')), $row['about']);
            	$row['name']     = str_replace($keyword, cl_html_el('span',$keyword,array('class' => 'query-match')), $row['name']);
            	$row['username'] = str_replace($keyword, cl_html_el('span',$keyword,array('class' => 'query-match')), $row['username']);
            }

...
function cl_is_logged($platform = "web") {
    if (isset($_POST['access_token'])) {
        $id = cl_get_userfromsession_id($_POST['access_token'], $platform);
    ...
    if (isset($_SESSION['user_id']) && not_empty($_SESSION['user_id'])) {
        $id = cl_get_userfromsession_id($_SESSION['user_id']);
        if (is_numeric($id) && not_empty($id)) {
            return true;
        }
    } 
...
function cl_total_new_notifs() {
    ...
    $db = $db->where('recipient_id', $cl['me']['id']);
...
function cl_total_new_messages() {
    ...
    $db = $db->where('sent_to', $cl['me']['id']);
    $db = $db->where('seen', '0');
    $qr = $db->getValue(T_MSGS, 'COUNT(*)');
